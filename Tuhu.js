 ...
 * 脚本名称：途虎 · 签到小报告 🐯
 * 活动规则：每日签到可获取积分奖励
 * 脚本说明：专为青龙面板修改适配，添加了调侃式幽默通知。支持多账号。
 * 环境变量：TUHU_TOKEN (必填，多个账号用 @ 或 & 分割)
 * * 修改者：Gemini
 */

const $ = new Env('途虎 · 签到小报告 🐯');
...
$.tokenArr = $.toObj($.token) || $.token.split(/[@&]|\\n/) || [];
...
async function main() {
    ...
    if ($.tokenArr.length) {
        $.log(`🐾 抓到 ${$.tokenArr.length} 只野生 Token ✅`);
        for (let i = 0; i < $.tokenArr.length; i++) {
            ...
            if (!$.is_login) {
                $.messages.push(`❌ 账号 [${i + 1}]: 登录失效了喂！是不是被途虎踢下线了？`);
                continue;  // 无效 token 跳出
            }
        ...
        throw new Error('未找到 TUHU_TOKEN 环境变量，巧妇难为无米之炊呀 ❌');
    }
}
...
async function getToken() {
    const options = {
        ...
        body: $.toStr({ channel: "WXAPP", code: $.wx_code })
    }
...
async function whoami() {
    ...
    if (result?.code == 10000 && result?.data) {
        ...
        msg += `🛵 尊贵的途虎大怨种 (划掉) 车主: [${nickName || mobile}] 闪亮登场！`;
    } else if (/token无效/.test($.toStr(result))) {
        $.is_login = false;
        msg += `⚠️ 警报: 身份验证失败，Token 似乎已经随风飘散了 ❌`;
    } else {
        $.log($.toStr(result));
    }
...
async function checkin(type, name) {
    ...
    let opt = {
        ...
        body: $.toStr({ "channel": type })
    };

    var result = await Request(opt);
    if (result?.data?.rewardIntegral) {
        msg += `✅ [${name}白嫖]: 搞定！含泪赚取 ${result?.data?.rewardIntegral} 积分，已苟延残喘连续签到 ${result?.data?.continuousDays}/7 天~`;
    } else {
        msg += `❌ [${name}翻车]: 没签上！是不是已经被你薅秃了？(${result?.message || '未知神秘力量拦截'})`;
    }
...
async function getIntegral() {
    ...
    const result = await Request(options);
    if (result?.Code == 1) {
        msg += `💰 [家底曝光]: 兜里还剩 ${result.IntegralNumber} 个积分，约等于巨款 ${result.IntegralNumber / 100} 元！（够买个轮胎气门嘴不？）\n`;
    } else {
        msg += `❌ 积分查询失败，可能是穷到连服务器都不忍心显示了。`;
    }
    $.messages.push(msg), $.log(msg);
}
...
async function getBlackBox() {
    try {
        ...
        const result = await Request(options);
        $.blackbox = result?.blackBox || $.blackbox;
    } catch (e) {
        $.log("❌ blackBox获取失败，使用兜底参数"), $.log(e);
    }
}

// 获取微信 Code (青龙面板环境一般用不到，保留作兼容)
async function getWxCode() {
    try {
        ...
        if (!$.codeServer) return; // $.log(`⚠️ 未配置微信 Code Server，跳过自动获取。`);
...
function hideSensitiveData(string, head_length = 2, foot_length = 2) {
    try {
        let star = '';
        for (var i = 0; i < string.length - head_length - foot_length; i++) star += '*';
...
function ObjectKeys2LowerCase(obj) {
    ...
    return new Proxy(_lower, {
        get: function (target, propKey, receiver) { return Reflect.get(target, propKey.toLowerCase(), receiver) },
        set: function (target, propKey, value, receiver) { return Reflect.set(target, propKey.toLowerCase(), value, receiver) }
    })
}
